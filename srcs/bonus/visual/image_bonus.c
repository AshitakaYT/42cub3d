...
/*   Updated: 2022/12/04 14:26:12 by aucousin         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_create_sprites(t_cube *c)
{
	...
	c->ma->po = ft_new_sprite(c->mlx, c->el->po);
	c->ma->fps_banner = ft_new_sprite(c->mlx, "./imgs/banner_fps.xpm");
	if (!c->ma->no.img || !c->ma->ea.img || !c->ma->we.img
		|| !c->ma->so.img || !c->ma->po.img || !c->ma->fps_banner.img)
		return (-1);
	return (0);
}
