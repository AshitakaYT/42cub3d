...
/*   Updated: 2022/12/07 16:31:28 by aucousin         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_readfilecub(char *path)
{
	...
	while (read(fd, &buf, 1))
	{
		...
		if (size > 100000)
		{
			ft_dprintf(2, "error: please input a 1000x100 max map.\n");
			close (fd);
			return (NULL);
		}
	}
...
int	cube_initparsing(t_cube *c)
{
	...
	tmp = c->map;
	if (ft_tablen(c->map) < 9)
		return (ft_printreturn("invalid map.\n", 2, -1));
	cube_malloc_elem(c);
	c->el->elems = ft_subtab(c->map, 0, 6);
	if (!c->el->elems)
		return (ft_printreturn("subtab failed.\n", 2, -1));
	c->map = ft_subtab(tmp, 6, ft_tablen(tmp) - 6);
	ft_freetab(tmp);
	if (!c->map)
		return (ft_printreturn("subtab failed.\n", 2, -1));
	return (0);
